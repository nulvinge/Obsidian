...
getPoint :: IO ((Float,Float,Float,Float), (Float,Float,Float))
getPoint = getPointRing
...
getPointRand = do
  ...
  let w = 0.1
  -- w <- randomRIO (0.1,1 :: Float)
  vx <- randomRIO (-1,1 :: Float)
  vy <- randomRIO (-1,1 :: Float)
  vz <- randomRIO (-1,1 :: Float)
  let d = x*x+y*y+z*z
  let d2 = vx*vx+vy*vy+vz*vz
  if d >= 1 || d2 >= 1
    then getPointRand
    else return ((x*ds,y*ds,z*ds,w),(vx*vs,vy*vs,vz*vs))

getPointRing = do
  deg <- randomRIO (0,3.14*2 :: Float)
  let x = sin(deg)
      y = cos(deg)
  z <- randomRIO (-0.1,0.1 :: Float)
  d <- randomRIO (0.5,1.0 :: Float)
  --let w = 0.1
  w <- randomRIO (0.05,0.2 :: Float)
  return ((x*d*ds,y*d*ds,z*ds,w), (y*d*ds*vs,-x*d*ds*vs,0))

getPointShell = do
  x <- randomRIO (-1,1 :: Float)
  y <- randomRIO (-1,1 :: Float)
  z <- randomRIO (-1.0,1.0 :: Float)
  dx <- randomRIO (2.5,4 :: Float)
  dy <- randomRIO (2.5,4 :: Float)
  dz <- randomRIO (2.5,4 :: Float)
  let w = 0.1
  --w <- randomRIO (0.1,1 :: Float)
  let d = x*x+y*y+z*z
  if d >= 1
    then getPointShell
    else return ((x*dx*ds,y*dy*ds,z*dz*ds,w),(y*dy*ds,-x*dx*ds,0))
    ...
    idle tick frameTick time state dispData = do
      ...
      if t `mod` 1 /= 0
        then return ()
          ...
          -}
            let t = fromRational $ toRational time
            putStrLn $ "bench\tfast\t" ++ show (fromIntegral frames / t)
                    ++ "\t" ++ show frames
                    ++ "\t" ++ show t
                    ++ "\t" ++ show flop
                    ++ "\t" ++ show (flop / t)
...
runVector = do 
  ...
  withCUDA True $ do
    ...
    let p1 = L.concat $ map (\((x,y,z,w),_) -> [x,y,z,w]) $ points
        p2 = fmap (*0) p1
        v  = L.concat $ map (\(_,(x,y,z)) -> [x,y,z]) $ points
...
runNormal = do 
  ...
  withCUDA True $ do
    ...
    let (x,y,z,w) = L.unzip4 $ map fst $ points
        x0 = map (*0) x
        (vx,vy,vz) = L.unzip3 $ map snd $ points

    useVectors [w,x,y,z,x0,x0,x0,vx,vy,vz,x0,x0,x0] $ \[w,x1,y1,z1,x2,y2,z2,vx1,vy1,vz1,vx2,vy2,vz2] -> do
      pingPongLoop (x1,y1,z1,vx1,vy1,vz1) (x2,y2,z2,vx2,vy2,vz2) display (length points) $ \(x1,y1,z1,vx1,vy1,vz1) (x2,y2,z2,vx2,vy2,vz2) -> do
        ...
        let dispD = zip3 px py pz
            size :: Int
            size = 1024
            mc x = ceiling $ ((x+1)/2)*fromIntegral size
            mcol z = ceiling $ min 255 $ max 16 $ 64*((z+1)/2)
            l = [((x,y,3), 255)
                | x <- A.range (0,size-1)
                , y <- A.range (0,size-1)
                ] ++ (concat $
                ...
                | (x,y,z) <- dispD
                , A.inRange (0,size-1) $ mc x
                , A.inRange (0,size-1) $ mc y
                ])
            f x y = min 255 (x+y)
            a :: A.UArray (Int, Int, Int) Word8
            a = A.accumArray f 0 ((0,0,0),(size-1,size-1,3)) l
        time <- lift $ getCurrentTime
        lift $ DevIL.writeImage ("/home/idk/prj/movie/run1-"++show time++".png") a
        return dispD
  where
